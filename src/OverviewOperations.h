...
CommandResult BuildMcpOverviewResult(const AgentConfig & config) {
    ...
    result.fields["local_ai_code_format_policy"] =
        "for C/C++ source whitespace, extra blank lines, indentation, or newline cleanup use lan_agent_format_code_file: dry_run=true first, then dry_run=false when would_change=true; do not scan comments or delete text ranges for formatting";
    result.fields["local_ai_long_loop_policy"] =
        "when continuation may exceed model context, freeze task_memory once; the chat/client must reset to a fresh context, then call lan_agent_task_memory_resume_and_execute with goal_id; MCP cannot erase host chat history itself";
    ...
    result.fields["local_ai_context_bootstrap_json"] =
        ...
        "\"include\":[\"user_goal\",\"goal_id\",\"trace_id\",\"current_file\",\"required_tool_name\",\"required_tool_arguments_json\",\"clean_chat_close_allowed\",\"chat_context_reset_required\",\"chat_context_reset_acknowledged\",\"old_context_dropped\",\"new_chat_entry_arguments_json\",\"completion_gate\",\"result_ref\",\"evidence_ref\"],"
