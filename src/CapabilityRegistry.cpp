...
const std::vector<SemanticActionSpec> & GetSemanticActionSpecs() {
    static const std::vector<SemanticActionSpec> actions = {
        ...
        {
            "scan_text_ranges",
            "Scan one text file server-side and return paged ranges such as comment spans before any edit planning.",
            "lan_agent_scan_text_ranges",
            "{\"file_path\":\"required string\",\"scan_mode\":\"optional string: comments|line_comments|block_comments\",\"max_ranges_per_call\":\"optional integer\",\"range_offset\":\"optional integer\",\"primary_intent\":\"optional string\",\"trace_id\":\"optional string\"}",
            "{\"file_path\":\"non_empty\",\"ranges_json\":\"non_empty when matches exist\",\"analysis_allowed\":\"true|false\",\"task_completion\":\"complete|incomplete\"}",
        ...
        },
        {
            ...
            "Build localized edit windows from ranges_json so downstream patching can stay segmented instead of reading the whole file.",
            "lan_agent_prepare_edit_windows",
            "{\"file_path\":\"required string\",\"ranges_json\":\"required JSON array string\",\"context_before\":\"optional integer\",\"context_after\":\"optional integer\",\"max_windows_per_call\":\"optional integer\",\"window_offset\":\"optional integer\",\"max_window_chars\":\"optional integer\",\"trace_id\":\"optional string\"}",
            "{\"file_path\":\"non_empty\",\"windows_json\":\"non_empty when windows are produced\",\"analysis_allowed\":\"true|false\",\"task_completion\":\"complete|incomplete\"}",
        ...
        },
        {
            ...
            "{\"file_path\":\"required string\",\"anchor_text\":\"required string or query_text/text/anchor\",\"occurrence\":\"optional integer\",\"expected_anchor_hash\":\"optional string\",\"request_id\":\"optional string\",\"trace_id\":\"optional string\"}",
        ...
        {
            "write_document",
            "Create, overwrite, or append exact text content to one remote document on the local filesystem. Probe the target file first when updating an existing large file. For larger scripts prefer content_base64 to avoid JSON escaping pollution.",
        ...
        {
            "refactor_file",
            "Preview and apply one high-risk single-file patch on the local filesystem with explicit trace fields, hash checks, backup, post-verify expectations, base64 content option, and compact large diff responses.",
        ...
        {
            "apply_diff_patch",
            "Apply one git-style unified diff to a single local file using controlled git apply --check/git apply first, then deterministic internal patching as fallback.",
